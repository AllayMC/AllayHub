...
const toggleLineStart = (
  ...
  if (shouldRemove) {
    ...
    const modifiedText = selectedText
      ...
      .replaceAll(lineBreak + text, lineBreak)
    transaction = state.update({
      ...
      selection: {
        ...
        head:
          ...
          text.length * numOfSelectedLinesThatNeedToBeRemoved,
      },
    })
  } else {
    const modifiedText =
      text + selectedText.replaceAll(lineBreak, lineBreak + text)
