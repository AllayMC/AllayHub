...
export const external = (cosmetics) =>
...
export const fileIsValid = (file, validationOptions) => {
  ...
  if (maxSize !== null && maxSize !== undefined && file.size > maxSize) {
    if (alertOnInvalid) {
      alert(
        `File ${file.name} is too big! Must be less than ${formatBytes(maxSize)}`,
      )
    }
    return false
  }
...
export const acceptFileFromProjectType = (projectType) => {
  const commonTypes =
    '.sig,.asc,.gpg,application/pgp-signature,application/pgp-keys'
  switch (projectType) {
    case 'mod':
