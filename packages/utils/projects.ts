...
export function formatVersionsForDisplay(
  ...
  // For semver ranges (e.g., ">=0.16.0", "^0.15.0"), return them directly without formatting
  const semverRanges = inputVersions.filter((v) => /[>=<^~]/.test(v))
  if (semverRanges.length > 0) {
    return semverRanges
  }
  ...
  const allSnapshots = allVersions.filter(
    (version) => version.version_type === 'snapshot',
  )
  ...
  const allLegacy = allVersions.filter(
    (version) =>
      version.version_type !== 'snapshot' && version.version_type !== 'release',
  )
  ...
  const dateString = allReleases.find(
    (version) => version.version === releaseVersions[0],
  )?.date
    ...
    allSnapshots.some(
      (gameVer) =>
        ...
        (!latestReleaseVersionDate ||
          latestReleaseVersionDate < Date.parse(gameVer.date)),
    ),
  )
  ...
  const releaseVersionsAsRanges = projectVersionsGrouped.map(
    ({ major, minor }) => {
      if (minor.length === 1) {
  ...
  const legacyVersionsAsRanges = groupConsecutiveIndices(
    inputVersions.filter((projVer) =>
      allLegacy.some((gameVer) => gameVer.version === projVer),
    ),
    allLegacy,
  )
  ...
  return versions
    ...
    .reduce((ranges: VersionRange[], version: string) => {
      ...
      if (matchesVersion) {
        ...
        const minorNumeric = minorVersion
          ? parseInt(minorVersion.replace('.', ''))
          : 0
