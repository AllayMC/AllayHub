<template>
  ...
  >
    <transition-group name="notifs">
      ...
      >
        <div
          class="flex w-full gap-2 overflow-hidden rounded-lg bg-bg-raised shadow-xl"
        >
          <div
            class="w-2"
            :class="{
              ...
              'bg-blue':
                !item.type ||
                !['error', 'warning', 'success'].includes(item.type),
            }"
          ></div>
          <div
          ...
          >
            ...
            <div class="flex items-center gap-1">
              <div
                ...
                class="text-xs font-bold text-contrast"
              >
                x{{ item.count }}
              </div>
            ...
            <div></div>
            <div
              class="col-span-2 text-sm text-primary"
              v-html="item.text"
            ></div>
...
const notificationLocation = computed(() =>
  notificationManager.getNotificationLocation(),
)

const isIntercomPresent = ref<boolean>(false)
const copied = ref<Record<string, boolean>>({})

const stopTimer = (n: WebNotification) =>
  notificationManager.stopNotificationTimer(n)
const setNotificationTimer = (n: WebNotification) =>
  notificationManager.setNotificationTimer(n)
const dismissNotification = (n: number) =>
  notificationManager.removeNotificationByIndex(n)
...
function checkIntercomPresence(): void {
  isIntercomPresent.value = !!document.querySelector(
