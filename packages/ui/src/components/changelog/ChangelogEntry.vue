  ...
  <div>
    <div class="flex items-center gap-4">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <AutoLink
          :to="
            hasLink
              ? `/news/changelog/${entry.product}/${entry.version ?? entry.date.unix()}`
              : ''
          "
          :class="{ 'hover:underline': hasLink }"
        >
          <h2
            class="m-0 flex items-center gap-2 text-xl font-extrabold text-contrast"
          >
            <template v-if="showType">
              {{ formatMessage(messages[entry.product]) }}
              <div class="h-2 w-2 rounded-full bg-secondary" />
            </template>
            <span :class="{ 'font-bold text-primary': showType }">
              {{ versionName }}
            </span>
          </h2>
        </AutoLink>
    ...
    <div class="ml-8 mt-3 rounded-2xl bg-bg-raised px-4 py-3">
      <div
        ...
        v-html="renderHighlightedString(entry.body)"
      />
    </div>
  </div>
</template>
...
const dateTooltip = computed(() =>
  props.entry.date.format('MMMM D, YYYY [at] h:mm A'),
)
