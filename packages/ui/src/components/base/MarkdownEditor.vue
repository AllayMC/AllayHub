      ...
      <div class="input-group push-right">
        ...
        <Button
          ...
          :action="
            () => {
              if (editor)
                markdownCommands.replaceSelection(editor, linkMarkdown)
              linkModal?.hide()
            }
          "
    ...
    <div class="modal-insert">
      <label class="label" for="insert-image-alt">
        <span class="label__title"
          >Description (alt text)<span class="required">*</span></span
        >
        <span class="label__description">
          Describe the image completely as you would to someone who could not
          see the image.
        </span>
      </label>
      <div class="iconified-input">
      ...
      <div class="input-group push-right">
        ...
        <Button
          ...
          :action="
            () => {
              if (editor)
                markdownCommands.replaceSelection(editor, imageMarkdown)
              imageModal?.hide()
            }
          "
        >
  ...
  <NewModal ref="videoModal" header="Insert YouTube video">
    <div class="modal-insert">
      <label class="label" for="insert-video-url">
        <span class="label__title"
          >YouTube video URL<span class="required">*</span></span
        >
        <span class="label__description">
      ...
      <div class="input-group push-right">
        ...
        <Button
          ...
          :action="
            () => {
              if (editor)
                markdownCommands.replaceSelection(editor, videoMarkdown)
              videoModal?.hide()
            }
          "
        >
  ...
  <div class="resizable-textarea-wrapper">
    <div class="editor-action-row">
      <div class="editor-actions">
        <template
          v-for="(buttonGroup, _i) in Object.values(BUTTONS).filter(
            (bg) => bg.display,
          )"
          :key="_i"
        >
...
import {
  markdownCommands,
  modrinthMarkdownEditorKeymap,
} from '@modrinth/utils/codemirror'
...
onMounted(() => {
  ...
  const theme = EditorView.theme({
    ...
    '&.cm-focused': {
      'box-shadow':
        'inset 0 0 0 transparent, 0 0 0 0.25rem var(--color-brand-shadow)',
  ...
  headings: {
    ...
    buttons: [
      composeCommandButton(
        'Heading 1',
        Heading1Icon,
        markdownCommands.toggleHeader,
      ),
      composeCommandButton(
        'Heading 2',
        Heading2Icon,
        markdownCommands.toggleHeader2,
      ),
      composeCommandButton(
        'Heading 3',
        Heading3Icon,
        markdownCommands.toggleHeader3,
      ),
    ],
  },
  stylizing: {
    ...
    buttons: [
      ...
      composeCommandButton(
        'Spoiler',
        ScanEyeIcon,
        markdownCommands.toggleSpoiler,
      ),
    ],
  },
  lists: {
  ...
  (newValue) => {
    if (editor) {
      if (isDisabledCompartment) {
        editor.dispatch({
          effects: [
            isDisabledCompartment.reconfigure(
...
const imageMarkdown = computed(() =>
  linkMarkdown.value.length ? `!${linkMarkdown.value}` : '',
)
...
const canInsertImage = computed(() => {
  ...
  return (
    !linkValidationErrorMessage.value &&
    linkUrl.value?.length > 0 &&
    linkText.value?.length > 0
  )
})
