<template>
  <div
    class="flex max-w-lg flex-col items-center rounded-3xl bg-bg-raised p-8 shadow-xl"
  >
    ...
    <div
      v-if="errorDetails"
      class="my-4 w-full rounded-lg border border-divider bg-bg-raised"
    >
      <div class="divide-y divide-divider">
        <div
          v-for="detail in errorDetails.filter(
            (detail) => detail.type !== 'hidden',
          )"
          :key="detail.label"
          class="px-4 py-3"
        >
          <div
            v-if="detail.type === 'inline'"
            class="flex items-center justify-between"
          >
            <span class="font-medium text-secondary">{{ detail.label }}</span>
            <div class="flex items-center gap-2">
              <code
                class="rounded-lg bg-code-bg px-2 py-1 text-sm text-code-text"
              >
                {{ detail.value }}
              </code>
            </div>
          ...
          <div v-else-if="detail.type === 'block'" class="flex flex-col gap-2">
            ...
            <div class="w-full overflow-hidden rounded-lg bg-code-bg p-3">
              <code
                class="block w-full overflow-x-auto whitespace-pre-wrap break-words text-sm text-code-text"
              >
                {{ detail.value }}
              </code>
            </div>
    ...
    <div class="mt-4 flex !w-full flex-row gap-4">
      ...
      >
        <button class="!w-full">
          <component
            :is="action.icon"
            v-if="action.icon && !action.showAltIcon"
            class="size-4"
          />
          <component
            :is="action.altIcon"
