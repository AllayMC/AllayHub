<template>
  <div ref="containerRef" class="relative inline-block w-full">
    <span
      ...
      role="button"
      tabindex="0"
      class="relative flex min-h-5 w-full cursor-pointer items-center justify-between overflow-hidden rounded-xl bg-button-bg px-4 py-2.5 text-left text-button-text transition-all duration-200 hover:bg-button-bgHover active:bg-button-bgActive"
      :class="[
        triggerClasses,
        {
          'z-[9999]': isOpen,
    ...
    <Teleport to="#teleports">
      <div
        v-if="isOpen"
        ref="dropdownRef"
        class="fixed z-[9999] flex flex-col overflow-hidden rounded-[14px] border-0 !border-solid bg-surface-4 shadow-2xl"
        :class="[
      ...
      >
        <div v-if="searchable" class="p-4">
          <div
            class="iconified-input w-full rounded-xl border-[1px] border-solid border-surface-5"
          >
        ...
        >
          <template v-for="(item, index) in filteredOptions" :key="item.key">
            ...
            <component
              ...
              :href="
                item.type === 'link' && !item.disabled ? item.href : undefined
              "
              :target="
                item.type === 'link' && !item.disabled ? item.target : undefined
              "
              :role="listbox ? 'option' : 'menuitem'"
              :aria-selected="listbox && item.value === modelValue"
              :aria-disabled="item.disabled || undefined"
              :data-focused="focusedIndex === index"
              class="flex cursor-pointer items-center gap-2.5 rounded-xl p-3 text-left text-contrast transition-colors duration-150 hover:bg-surface-5 focus:bg-surface-5"
        ...
        <div
          ...
          class="mb-2 p-4 text-center text-sm text-secondary"
        >
          {{ noOptionsMessage }}
        </div>
      </div>
...
const selectedOption = computed<ComboboxOption<T> | undefined>(() => {
  return props.options.find(
    (opt): opt is ComboboxOption<T> =>
      isDropdownOption(opt) && opt.value === props.modelValue,
  )
})
...
const shouldRoundBottomCorners = computed(
  () => isOpen.value && openDirection.value === 'down',
)
const shouldRoundTopCorners = computed(
  () => isOpen.value && openDirection.value === 'up',
)

function getOptionClasses(
  item: ComboboxOption<T> & { key: string },
  index: number,
) {
  return [
    item.class,
...
function setInitialFocus() {
  focusedIndex.value = props.listbox
    ? props.options.findIndex(
        (opt) => isDropdownOption(opt) && opt.value === props.modelValue,
      )
    : -1
...
function determineOpenDirection(
  ...
  const hasSpaceBelow =
    triggerRect.bottom + dropdownRect.height + DROPDOWN_VIEWPORT_MARGIN <=
    viewportHeight
  const hasSpaceAbove =
    triggerRect.top - dropdownRect.height - DROPDOWN_VIEWPORT_MARGIN > 0
...
async function updateDropdownPosition() {
  ...
  const direction = determineOpenDirection(
    triggerRect,
    dropdownRect,
    viewportHeight,
  )
...
function findNextFocusableOption(
  currentIndex: number,
