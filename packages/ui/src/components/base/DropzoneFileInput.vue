<template>
  <label
    :class="[
      'flex cursor-pointer flex-col items-center justify-center border-2 border-dashed bg-surface-4 text-contrast transition-colors',
      size === 'small' ? 'p-5' : 'p-12',
  ...
  >
    <div
      :class="[
        'grid place-content-center border border-solid border-brand bg-highlight-green text-brand',
    ...
    >
      <FolderUpIcon
        aria-hidden="true"
        :class="['text-brand', size === 'small' ? 'h-6 w-6' : 'h-8 w-8']"
      />
    </div>

    <div
      class="flex flex-col items-center justify-center gap-1 text-center text-contrast"
    >
      <div class="text-pretty font-medium text-contrast">
        {{ primaryPrompt }}
      </div>
      <span class="text-pretty text-sm text-primary">
        {{ secondaryPrompt }}
      </span>
    </div>
...
const props = withDefaults(
  ...
  }>(),
  {
    ...
    secondaryPrompt:
      'You can try to drag files or folder or click this area to select it',
    size: 'standard',
  },
)
