<template>
  <Accordion
    ...
    ref="accordion"
    :button-class="
      buttonClass ?? 'flex flex-col gap-2 justify-start items-start'
    "
    :content-class="contentClass"
    title-wrapper-class="flex flex-col gap-2 justify-start items-start"
    :open-by-default="
      !locked && (openByDefault !== undefined ? openByDefault : true)
    "
  >
    <template #title>
      ...
      v-if="
        ...
        (!!accordion &&
          ...
          (selectedFilterOptions.length > 0 ||
            selectedNegativeFilterOptions.length > 0))
      "
      #summary
    >
      <div class="flex flex-wrap gap-1">
        <div
          ...
          class="flex w-fit shrink-0 items-center gap-1 rounded-full bg-button-bg px-2 py-0.5 text-xs font-bold text-secondary"
        >
    ...
    <template v-if="locked" #default>
      <div
        class="mx-2 flex flex-col gap-2 rounded-2xl border-2 border-dashed border-divider p-3"
      >
        <p class="m-0 items-center font-bold">
          <slot :name="`locked-${filterType.id}`">
            {{
              formatMessage(messages.lockedTitle, {
                type: filterType.formatted_name,
              })
            }}
          </slot>
        </p>
    ...
    <template v-else #default>
      ...
      <ScrollablePanel
        :class="{ 'h-[16rem]': scrollable }"
        :disable-scrolling="!scrollable"
      >
        <div
        ...
        >
          ...
          <button
            ...
            class="flex !w-full cursor-pointer items-center gap-2 truncate rounded-xl border-none bg-transparent px-2 py-1 text-sm font-semibold text-secondary transition-all hover:text-contrast focus-visible:text-contrast active:scale-[0.98]"
      ...
      <div :class="innerPanelClass ? innerPanelClass : ''" class="empty:hidden">
        ...
        />
        <div v-if="hasProvidedFilter" class="mx-1 mt-2">
          <ButtonStyled>
            <button
              class="w-fit"
              @click="
                () => {
                  overriddenProvidedFilterTypes =
                    overriddenProvidedFilterTypes.filter(
                      (id) => id !== filterType.id,
                    )
                  accordion?.close()
                  clearFilters()
...
const selectedFilters = defineModel<FilterValue[]>('selectedFilters', {
  required: true,
})
const toggledGroups = defineModel<string[]>('toggledGroups', { required: true })
const overriddenProvidedFilterTypes = defineModel<string[]>(
  'overriddenProvidedFilterTypes',
  {
    required: false,
    default: [],
...
const visibleOptions = computed(() =>
  props.filterType.options
    .filter(
      (option) => isVisible(option) || isIncluded(option) || isExcluded(option),
    )
    .slice()
    .sort((a, b) => {
...
const locked = computed(
  () =>
    hasProvidedFilter.value &&
    !overriddenProvidedFilterTypes.value.includes(props.filterType.id),
)

const scrollable = computed(
  () =>
    visibleOptions.value.length >= 10 &&
    props.filterType.display === 'scrollable',
)
...
function isIncluded(filter: FilterOption) {
  return selectedFilters.value.some(
    (value) => value.option === filter.id && !value.negative,
  )
}
...
function isVisible(filter: FilterOption) {
  const filterKey =
    filter.formatted_name?.toLowerCase() ?? filter.id.toLowerCase()
  const matchesQuery =
    !query.value || filterKey.includes(query.value.toLowerCase())

  if (props.filterType.display === 'expandable') {
    return (
      matchesQuery &&
      (showMore.value || props.filterType.default_values.includes(filter.id))
    )
  }
...
function toggleFilter(filter: FilterOption) {
  setFilter(
    filter,
    isIncluded(filter) || isExcluded(filter) ? 'ignore' : 'include',
  )
}
...
function setFilter(filter: FilterOption, state: FilterState) {
  const newFilters = selectedFilters.value.filter(
    (selected) => selected.option !== filter.id,
  )
...
const messages = defineMessages({
  ...
  lockedDescription: {
    ...
    defaultMessage:
      'Unlocking this filter may allow you to install incompatible content.',
  },
})
</script>
