<template>
  <div
    class="experimental-styles-within flex flex-wrap items-center gap-1 empty:hidden"
  >
    <TagItem
...
function filterMatches(
  ...
  return list.some(
    (provided) => provided.type === type.id && provided.option === option.id,
  )
}
  ...
  return props.filters.flatMap((type) =>
    type.options
      ...
      .map((option) => ({
        type: type.id,
        option: option.id,
        negative: selectedFilters.value.find(
          (x) => x.type === type.id && x.option === option.id,
        )?.negative,
