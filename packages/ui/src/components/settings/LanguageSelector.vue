...
const $locales = computed(() => {
  ...
  for (const loc of props.locales) {
    ...
    const nativeDisplayNames = new Intl.DisplayNames([tag], {
      type: 'language',
    })
...
<template>
  <div class="flex flex-col gap-4">
    <div v-if="$locales.length > 1" class="iconified-input -mb-4 w-full">
      <SearchIcon />
      <input
        id="language-search"
        v-model="$query"
    ...
    <div ref="$languagesList" class="flex flex-col gap-2.5">
      <template
        v-for="[category, categoryLocales] in $displayCategories"
        :key="category"
      >
        ...
        <template v-for="loc in categoryLocales" :key="loc.tag">
          <div
            ...
            :class="[
              'relative flex cursor-pointer items-center gap-2 overflow-hidden rounded-lg border-2 border-transparent bg-surface-4 p-4 py-2 transition-colors duration-100',
              'border-solid hover:border-surface-5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand',
