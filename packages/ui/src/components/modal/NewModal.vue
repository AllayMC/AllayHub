<template>
  ...
  >
    ...
    <div
      ...
      :class="{ shown: visible }"
    >
      <div class="modal-body flex flex-col rounded-2xl bg-bg-raised">
        <div
          v-if="!hideHeader"
          data-tauri-drag-region
          class="grid max-w-full grid-cols-[auto_min-content] items-center gap-12 border-0 border-b-[1px] border-solid border-divider p-6"
        >
          <div class="flex min-w-0 items-center gap-3 text-wrap break-words">
            <slot name="title">
...
.modal-overlay {
  ...
  &.standard {
    background: linear-gradient(
      to bottom,
      rgba(29, 48, 43, 0.52) 0%,
      rgba(14, 21, 26, 0.95) 100%
    );
  }
  ...
  &.warning {
    background: linear-gradient(
      to bottom,
      rgba(48, 38, 29, 0.52) 0%,
      rgba(26, 20, 14, 0.95) 100%
    );
  }
  ...
  &.danger {
    background: linear-gradient(
      to bottom,
      rgba(43, 18, 26, 0.52) 0%,
      rgba(49, 10, 15, 0.95) 100%
    );
  }
