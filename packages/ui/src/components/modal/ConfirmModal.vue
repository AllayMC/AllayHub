<template>
  <NewModal ref="modal" :noblur="noblur" :danger="danger" :on-hide="onHide">
    <template #title>
      <slot name="title">
        <span class="text-lg font-extrabold text-contrast">{{ title }}</span>
      </slot>
    </template>
    <div class="flex flex-col gap-4">
      <template v-if="description">
        ...
        />
        <p v-else class="m-0 max-w-[35rem]">
          {{ description }}
        </p>
      </template>
      <slot />
      <label v-if="hasToType" for="confirmation">
        <span>
          ...
          <span class="font-bold italic">{{ confirmationText }}</span> below:
        </span>
      </label>
      <input
        v-if="hasToType"
...
const action_disabled = computed(
  () =>
    props.hasToType &&
    confirmation_typed.value.toLowerCase() !==
      props.confirmationText.toLowerCase(),
)
