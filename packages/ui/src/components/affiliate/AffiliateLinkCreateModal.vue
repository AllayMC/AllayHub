  ...
  <NewModal ref="modal" :header="formatMessage(messages.createHeader)">
    <div class="flex flex-col">
      <label
        v-if="showUserField"
        class="contents"
        for="create-affiliate-user-input"
      >
        <span class="mb-1 text-lg font-semibold text-contrast">
          {{ formatMessage(messages.createUserLabel) }}
        </span>
        <span class="mb-2 text-secondary">{{
          formatMessage(messages.createUserDescription)
        }}</span>
      </label>
      <div v-if="showUserField" class="mb-4">
        <div class="iconified-input">
          ...
          />
          <Button
            v-if="affiliateUsername"
            class="r-btn"
            @click="() => (affiliateUsername = '')"
          >
            <XIcon />
          </Button>
        </div>
      ...
      <div class="flex items-center gap-2">
        <div class="iconified-input">
          ...
          />
          <Button
            v-if="affiliateLinkTitle"
            class="r-btn"
            @click="() => (affiliateLinkTitle = '')"
          >
            <XIcon />
          </Button>
        </div>
        <ButtonStyled color="brand">
          ...
          >
            ...
            {{
              formatMessage(
                creatingLink ? messages.creatingButton : messages.createButton,
              )
            }}
          </button>
        </ButtonStyled>
...
const messages = defineMessages({
  ...
  createTitleDescription: {
    ...
    defaultMessage:
      'Give your affiliate link a name so you know where people are coming from!',
  },
