...
const value = defineModel<Labrinth.Projects.v3.Environment | undefined>({
  required: true,
})
...
const OUTER_OPTIONS: Record<
  string,
  EnvironmentRadioOption & {
    suboptions: Record<string, EnvironmentRadioOption>
  }
> = {
  client: {
    title: defineMessage({
...
type SubOptionKey = ValidKeys<
  (typeof OUTER_OPTIONS)[keyof typeof OUTER_OPTIONS]['suboptions']
>
...
<template>
  <div
    role="radiogroup"
    :aria-label="formatMessage(commonProjectSettingsMessages.environment)"
  >
    <template
    ...
    >
      ...
      <div
        ...
        role="radiogroup"
        :aria-label="
          formatMessage(subOptionLabel, { option: formatMessage(title) })
        "
      >
        <LargeRadioButton
          v-for="(
            { title: suboptionTitle, description: suboptionDescription },
            suboptionKey
          ) in suboptions"
