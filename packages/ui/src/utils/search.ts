...
export function useSearch(
  ...
  const filters = computed(() => {
    ...
    for (const category of sortByNameOrNumber(tags.value.categories.slice(), [
      'header',
      'name',
    ])) {
    ...
    const filterTypes: FilterType[] = [
      ...
      },
      {
        ...
        options: tags.value.gameVersions.map((gameVersion) => ({
          ...
          toggle_group:
            gameVersion.version_type !== 'release' ? 'all_versions' : undefined,
  ...
  const facets = computed(() => {
    ...
    const filteredFilters = currentFilters.value.filter(
      (userFilter) =>
        !validProvidedFilters.some(
          (providedFilter) => providedFilter.type === userFilter.type,
        ),
    )
    ...
    for (const filterValue of filterValues) {
      ...
      let option = type?.options.find(
        (option) => option.id === filterValue.option,
      )
    ...
    const projectType = projectTypes.value.map(
  ...
  const requestParams: Ref<string> = computed(() => {
    const params = [
      ...
      `index=${currentSortType.value.name}`,
    ]
  ...
  function readQueryParams() {
    ...
    loadQueryParam(['l'], (openSource) => {
      if (
        ...
        !currentFilters.value.some(
          (filter) =>
            filter.type === 'license' && filter.option === 'open_source',
        )
      ) {
    ...
    loadQueryParam(['nf'], (filter) => {
      const set =
        typeof filter === 'string' ? new Set([filter]) : new Set(filter)
      ...
      typesLoop: for (const type of filters.value) {
        for (const option of type.options) {
          ...
          if (
            ...
            !currentFilters.value.some(
              (filter) =>
                filter.type === type.id && filter.option === option.id,
            )
          ) {
            currentFilters.value.push({
              type: type.id,
    ...
    loadQueryParam(['s'], (sort) => {
      currentSortType.value =
        sortTypes.find((sortType) => sortType.name === sort) ?? sortTypes[0]
      readParams.add('s')
    })
    ...
    for (const key of Object.keys(route.query).filter(
      (key) => !readParams.has(key),
    )) {
...
export function createEnvironmentFacets(
  client: boolean,
  server: boolean,
): string[][] {
  const facets: string[][] = []
...
function getOptionValue(option: FilterOption, negative?: boolean): string {
  let value =
    option.method === 'or' || option.method === 'and' ? option.value : option.id
  if (negative === true) {
    value = value.replace(':', '!=')
  }
...
function addQueryParam(
  ...
  return (
    existing + `${!existing ? '?' : '&'}${key}=${encodeURIComponent(value)}`
  )
}

function getParamValuesAsArray(
  x: LocationQueryValue | LocationQueryValue[],
): string[] {
  if (x === null) {
    return []
