...
const validateValues = <K extends PropertyKey>(flags: Record<K, FlagValue>) =>
  flags
...
export const useFeatureFlags = () =>
  useState<AllFeatureFlags>('featureFlags', () => {
    ...
    const savedFlags = useCookie<PartialFeatureFlags>(
      'featureFlags',
      COOKIE_OPTIONS,
    )
    ...
    const flags: AllFeatureFlags = JSON.parse(
