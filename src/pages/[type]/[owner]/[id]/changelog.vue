<template>
  <div class="content">
    ...
    <div class="card changelog-wrapper">
      <div
        v-for="version in filteredVersions.slice(
          (currentPage - 1) * 20,
          currentPage * 20,
        )"
        :key="version.id"
      ...
      >
        <div
          :class="`changelog-bar ${version.prerelease ? 'beta' : 'release'} ${version.duplicate ? 'duplicate' : ''}`"
        />
            ...
            <div class="version-header-text">
              ...
              <span v-if="version.author">
                ...
                <nuxt-link
                  ...
                  >{{ version.author.user.username }}</nuxt-link
                >
              </span>
              <span>
                on
                {{ $dayjs(version.published_at).format('MMM D, YYYY') }}</span
              >
            </div>
            <a
...
// Helper to get array or string from query param
function getArrayOrString(
...
// Extended version type with primaryFile for template usage
type VersionWithPrimaryFile = AllayIndex.Version & {
  primaryFile?: AllayIndex.VersionFile
  author?: AllayIndex.MemberView
  duplicate?: boolean
}

// Add primaryFile to each version for template usage
const versionsWithPrimaryFile = computed<VersionWithPrimaryFile[]>(() => {
  return props.versions.map((v) => ({
    ...v,
    primaryFile: v.files?.find((f) => f.primary) || v.files?.[0],
  }))
})

const filteredVersions = computed<VersionWithPrimaryFile[]>(() => {
  const selectedVersionTypes = getArrayOrString(route.query.c)

  return versionsWithPrimaryFile.value.filter(
    (projectVersion) =>
      selectedVersionTypes.length === 0 ||
      selectedVersionTypes.includes(
        projectVersion.prerelease ? 'beta' : 'release',
      ),
  )
})
