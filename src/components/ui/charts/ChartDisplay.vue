<template>
  <div>
    ...
    <div
      v-else-if="!isInitialized || analytics.loading.value"
      ...
      <div class="graphs__vertical-bar">
        <client-only>
          <CompactChart
            ...
            :value="
              formatNumber(analytics.formattedData.value.downloads.sum, false)
            "
        ...
        <client-only>
          <CompactChart
            ...
            :value="
              formatNumber(analytics.formattedData.value.views.sum, false)
            "
        ...
        <client-only>
          <CompactChart
            ...
            :value="
              formatMoney(analytics.formattedData.value.revenue.sum, false)
            "
          ...
          <div class="chart-controls">
            ...
            <div class="chart-controls__buttons">
              ...
              <DropdownSelect
                ...
                :display-name="
                  (o: RangeObject) =>
                    o?.getLabel([startDate, endDate]) ?? 'Loading...'
                "
              />
            </div>
          </div>
          <div class="chart-area">
            <div class="chart">
              <client-only>
                <Chart
                  v-if="
                    analytics.formattedData.value.downloads &&
                    selectedChart === 'downloads'
                  "
                  ref="downloadsChart"
                  type="line"
                  ...
                  :colors="
                    isUsingProjectColors
                      ...
                      : analytics.formattedData.value.downloads.chart
                          .defaultColors
                  "
                />
                <Chart
                  v-if="
                    analytics.formattedData.value.views &&
                    selectedChart === 'views'
                  "
                  ref="viewsChart"
                  type="line"
                ...
                <Chart
                  v-if="
                    analytics.formattedData.value.revenue &&
                    selectedChart === 'revenue'
                  "
                  ref="revenueChart"
                  type="line"
                  ...
                  :colors="
                    isUsingProjectColors
                      ...
                      : analytics.formattedData.value.revenue.chart
                          .defaultColors
                  "
                />
              </client-only>
        ...
        <div class="country-data">
          ...
          >
            ...
            <div class="country-values">
              <div
                v-for="[name, count] in analytics.formattedData.value
                  .downloadsByCountry.data"
                :key="name"
                class="country-value"
              >
                ...
                <div
                  v-tooltip="
                    `${
                      Math.round(
                        (count /
                          analytics.formattedData.value.viewsByCountry.sum) *
                          10000,
                      ) / 100
                    }%`
                  "
...
import {
  UiChartsChart as Chart,
  UiChartsCompactChart as CompactChart,
} from '#components'
import {
...
const selectedChart = computed({
  get: () => {
    const id =
      (router.currentRoute.value.query?.chart as string | undefined) ||
      'downloads'
...
const removeProjectFromDisplay = (id: string) => {
  selectedDisplayProjects.value = selectedDisplayProjects.value.filter(
    (p) => p.id !== id,
  )
}
...
onBeforeMount(() => {
  ...
  if (import.meta.client) {
    ...
    if (rangeLabel) {
      const range = props.ranges.find(
        (r) => r.getLabel([dayjs(), dayjs()]) === rangeLabel,
      )!
...
const onDownloadSetAsCSV = useClientTry(
  async () => await downloadSelectedSetAsCSV(),
)
