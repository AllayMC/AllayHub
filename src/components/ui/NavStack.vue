      ...
      <template v-else>
        ...
        >
          <hr
            ...
            class="my-1 w-full border-t border-solid"
          />
          ...
          >
            ...
            <span v-if="item.chevron" class="ml-auto"
              ><ChevronRightIcon
            /></span>
          </NuxtLink>
...
function getKey(item: NavStackEntry, idx: number) {
  ...
  return link
    ? `link-${link}`
    : `action-${(item as NavStackLinkItem).label}-${idx}`
}
...
const filteredItems = computed(() =>
  props.items?.filter((x) => x.shown === undefined || x.shown),
)
</script>
